...
public class Biomes_v1_17_R1 implements VersionAbove1_16 {
    @Override
    public boolean createBiome(MinecraftKey key, Biomes.BiomeBase base, Biomes.BiomeColor color, Biomes.Particle particle) {
        String root = "biomes." + key.getNamespace() + "." + key.getKey();

        Server server = Bukkit.getServer();
        CraftServer craftServer = (CraftServer) server;
        DedicatedServer dedicatedServer = craftServer.getServer();

        ResourceKey<BiomeBase> newKey = ResourceKey.a(IRegistry.aO, key);
        ResourceKey<BiomeBase> oldKey = ResourceKey.a(IRegistry.aO, new MinecraftKey("minecraft", "forest"));

        IRegistryWritable<BiomeBase> writableRegistry = (IRegistryWritable<BiomeBase>) dedicatedServer.getCustomRegistry().d(IRegistry.aO);
